...
import { getForcastAccuracyMaterialMonthwise } from '../../../../actions';
...
const AccuracyForecast = (props) => {
  const dispatch = useDispatch();

  const getForcastAccuracyMaterialMonthwiseData = useSelector(
    (state) => state.getForcastAccuracyMaterialMonthwise
  );

  const getForcastAccuracyMaterialMonthwiseLoaderReducer = useSelector(
    (state) => state.getForcastAccuracyMaterialMonthwiseLoaderReducer
  );
  const exportToCSVRadialAOMatnr = () => {
    let csvData = getForcastAccuracyMaterialMonthwiseData;
    let fileName = `${props.Material} (${props.LGORT}) -Forecast Accuracy`;
  ...
  useEffect(() => {
    dispatch(getForcastAccuracyMaterialMonthwise(props.Material, props.LGORT, 'all', 'all'));
  }, []);
  ...
  return (
    <>
      ...
      <Button
        size="sm"
        disabled={!getForcastAccuracyMaterialMonthwiseData.length > 0}
      ...
      <Row className="v4">
        ...
        getForcastAccuracyMaterialMonthwiseData.length > 0 ? (
          <ResponsiveContainer height={350} width="100%">
            <LineChart
              width={900}
              height={350}
              data={getForcastAccuracyMaterialMonthwiseData}
              margin={{
                top: 10,
                right: 30,
                left: 0,
...
export default AccuracyForecast;
